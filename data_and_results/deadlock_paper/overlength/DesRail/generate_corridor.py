...
def generate_corridor(num_loops, output_dir, corridor_len, loop_len, spawn_rate, horizon, seed,
                      corridor_len_var=0.0, warmup=0, pl_max_length=0.0, overlength=False):
    ...
    # Overlength: add separate spawn/exit segments for long trains so that
    # engineered constraints holding a long train don't block short train spawning
    seg_W_spawn_long = None
    seg_W_exit_long = None
    seg_E_spawn_long = None
    seg_E_exit_long = None
    node_W_spawn_long = None
    node_E_spawn_long = None
    if overlength:
        node_W_spawn_long = num_nodes; num_nodes += 1
        node_E_spawn_long = num_nodes; num_nodes += 1

        seg_W_spawn_long = seg_id
        segments.append((seg_id, node_W_spawn_long, node_W_junc, 1.0, 0, "West spawn long")); seg_id += 1
        seg_W_exit_long = seg_id
        segments.append((seg_id, node_W_junc, node_W_spawn_long, 0.5, 0, "West exit long")); seg_id += 1
        seg_E_spawn_long = seg_id
        segments.append((seg_id, node_E_spawn_long, node_E_junc, 1.0, 0, "East spawn long")); seg_id += 1
        seg_E_exit_long = seg_id
        segments.append((seg_id, node_E_junc, node_E_spawn_long, 0.5, 0, "East exit long")); seg_id += 1
    ...
    short_w_exit = "West_exit_short" if overlength else "West_exit"
    short_e_exit = "East_exit_short" if overlength else "East_exit"
    signals.append((seg_W_spawn, node_W_junc, 0.05, 0, "West_entry", "West spawn entry"))
    signals.append((seg_W_exit, node_W_spawn, 0.0, 0, short_w_exit, "West exit terminal"))

    # East terminal signals
    signals.append((seg_E_spawn, node_E_junc, 0.05, 0, "East_entry", "East spawn entry"))
    signals.append((seg_E_exit, node_E_spawn, 0.0, 0, short_e_exit, "East exit terminal"))

    # Long train terminal signals (if overlength)
    if overlength:
        signals.append((seg_W_spawn_long, node_W_junc, 0.05, 0, "West_entry", "West spawn long entry"))
        signals.append((seg_W_exit_long, node_W_spawn_long, 0.0, 0, "West_exit", "West exit long terminal"))
        signals.append((seg_E_spawn_long, node_E_junc, 0.05, 0, "East_entry", "East spawn long entry"))
        signals.append((seg_E_exit_long, node_E_spawn_long, 0.0, 0, "East_exit", "East exit long terminal"))
    ...
    for i in range(N):
        ...
        passing_loops.append((i, appr_west_seg, pw, 0.05, 40, "APPR0", 0.0, f"PL_{i} appr from west"))

        # APPR1: approach from east side, arriving at pe (feeds into ML1/S1 = westbound traversal)
        passing_loops.append((i, appr_east_seg, pe, 0.05, 40, "APPR1", 0.0, f"PL_{i} appr from east"))
        ...
        ml = pl_max_length
        passing_loops.append((i, main_seg, pw, 0.05, 0, "ML1", ml, f"PL_{i} ML WB"))
        passing_loops.append((i, main_seg, pe, 0.05, 0, "ML0", ml, f"PL_{i} ML EB"))
        ...
        passing_loops.append((i, side_seg, pw, 0.05, 20, "S1", ml, f"PL_{i} siding WB"))
        passing_loops.append((i, side_seg, pe, 0.05, 20, "S0", ml, f"PL_{i} siding EB"))

    with open(os.path.join(input_dir, "passing_loops.csv"), "w", newline="") as f:
        w = csv.writer(f)
        if pl_max_length > 0:
            w.writerow(["pl_id", "segment", "head", "margin", "opp_speed_lim", "arc_type", "max_length", "comment"])
        else:
            w.writerow(["pl_id", "segment", "head", "margin", "opp_speed_lim", "arc_type", "comment"])
        for pl in passing_loops:
            row = list(pl)
            if pl_max_length <= 0:
                del row[6]  # strip max_length column, keep comment
            w.writerow(row)
    ...
    # Long spawn/exit pairs (separate tracks, no violations with short spawn)
    if seg_W_spawn_long is not None:
        violations.append((seg_W_spawn_long, seg_W_exit_long))
        violations.append((seg_W_exit_long, seg_W_spawn_long))
        violations.append((seg_E_spawn_long, seg_E_exit_long))
        violations.append((seg_E_exit_long, seg_E_spawn_long))
    ...
    with open(os.path.join(input_dir, "train_templates.csv"), "w", newline="") as f:
        ...
        w.writerow(["freight", 80, 360, 720, 2, "loco", 20, "car",
                     "", "", "", "", "", ""])
        w.writerow(["freight_long", 80, 360, 720, 2, "loco", 38, "car",
                     "", "", "", "", "", ""])
    ...
    spawners = []
    if overlength:
        # Separate spawners for short and long trains; short trains use _short terminals
        spawners.append(("ols_EB", seg_W_spawn, node_W_junc, "freight", "exponential",
                         spawn_rate, 0, "NA", 0, "East_exit_short", 0))
        spawners.append(("ols_EB_long", seg_W_spawn_long, node_W_junc, "freight_long", "exponential",
                         spawn_rate, 0, "NA", 0, "East_exit", 0))
        spawners.append(("ols_WB", seg_E_spawn, node_E_junc, "freight", "exponential",
                         spawn_rate, 0, "NA", 0, "West_exit_short", 0))
        spawners.append(("ols_WB_long", seg_E_spawn_long, node_E_junc, "freight_long", "exponential",
    ...
    with open(os.path.join(input_dir, "runctrl.csv"), "w", newline="") as f:
        ...
        w.writerow(["pl_constraint_mode", 1, "segment-based"])
    ...
    with open(os.path.join(input_dir, "dlexp_options.csv"), "w", newline="") as f:
        ...
        w.writerow(["MERGE_AND_LOWER", 1, ""])
        w.writerow(["LOG_CONSTRAINT_FIRES", 0, ""])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--overlength", action="store_true",
                        help="Enable overlength train support: adds max_length to PL main/siding constraints")
    parser.add_argument("--pl_max_length", type=float, default=0.7,
                        help="Max train length for PL constraints when --overlength is set (default: 0.7)")
    ...
    generate_corridor(
        ...
        warmup=args.warmup,
        pl_max_length=args.pl_max_length if args.overlength else 0.0,
        overlength=args.overlength,
    )
